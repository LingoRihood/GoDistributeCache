...
func TestCacheBasic(t *testing.T) {
	...
	t.Run("添加和获取", func(t *testing.T) {
		...
		// "新值" 是一个 无类型（untyped）的 string 常量
		// 如果一个操作数是无类型常量，另一个操作数是某种确定类型 T，
		// 那编译器会尝试把这个常量转换成类型 T，只要能表示得下。
		// node.v.(testValue) != testValue("新值")
		// 是编译器自动帮你把右边的常量 "新值" 转成 testValue("新值")，所以两个同类型的值就可以直接比较了
...
// 测试LRU2Store的LRU替换策略(源代码并没有实现,这个测试是幻觉,这个测试函数可以忽略)
...
func TestLRU2StoreClear(t *testing.T) {
	...
	// BucketCount = 2所以maskOfNextPowOf2(2) = 1
	// 所以 idx = hash(key) & 1，只看 hash 的最低一位：
	// 为 0 → bucket 0
	// 为 1 → bucket 1
	/*
		key0 -> hash = 242294897 -> hash & 1 = 1
		key1 -> hash = 242294898 -> hash & 1 = 0
		key2 -> hash = 242294899 -> hash & 1 = 1
		key3 -> hash = 242294900 -> hash & 1 = 0
		key4 -> hash = 242294901 -> hash & 1 = 1
		key5 -> hash = 242294902 -> hash & 1 = 0
		key6 -> hash = 242294903 -> hash & 1 = 1
		key7 -> hash = 242294904 -> hash & 1 = 0
		key8 -> hash = 242294905 -> hash & 1 = 1
		key9 -> hash = 242294906 -> hash & 1 = 0
	*/
	store := newLRU2Cache(opts)
	defer store.Close()
...
func TestLRU2StoreHitRatio(t *testing.T) {
	...
	// 验证命中率大致为0.25-0.5（因为我们添加了50个项但有分桶和LRU淘汰）
	if hitRatio < 0.25 || hitRatio >= 0.5 {
...
func BenchmarkLRU2StoreOperations(b *testing.B) {
	...
	// 确保计时从这一点开始。在填充数据后，执行这一步，避免填充数据的时间影响基准测试的结果
	b.ResetTimer()
